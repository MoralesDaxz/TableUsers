...
const columna = [
    columnHelper.accessor(row => row.picture?.medium, {
        ...
        cell: info => <div id='img' className='flex items-center justify-center'>
            <Image src={`${info.row.original.picture?.medium}`} width={40} height={40} alt='imagen' />
        </div>,
    }),
                                ...
                                return (
                                    ...
                                    >
                                        <div className='flex items-center justify-center'>
                                            {/* Iconos Sort */}
                                            {isSortable && sort === "asc"
                                                ? <Image src={iconDown} width={24} height={24} alt='Icon' />
                                                : isSortable && sort === "desc"
                                                    ? <Image src={iconDown} className=' rotate-180' width={24} height={24} alt='Icon' />
                                                    : ''}
                        ...
                        return (
                            <tr className={index % 2 == 0 ? 'bg-[#161616] hover:bg-[#172d3a]' : 'hover:bg-[#172d3a]'} key={row.id}>
                                {row.getVisibleCells().map(cell => (
                                    <td onClick={(e) => findUser(row.original.login?.uuid)} className={'py-2 px-1 border border-[#AFAFAF] hover:cursor-pointer'} key={cell.id}>
